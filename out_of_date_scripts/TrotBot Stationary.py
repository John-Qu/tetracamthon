...
bar10 = 1
oldP = 1
bar11 = -2.64 #this bar is negative because we use the line extension to extend back into the bar.
bar12 = 2.55
bar13 =7.2
bar14 = 1
bar15 = 7.55
...
def init():
    for i in range(len(j_7) / footSweepIncrements):
        xLeg7 = []
        yLeg7 = []
        xLeg12 = []
        yLeg12 = []
        for j in range(i + 1):
            xLeg7.append(xVals[str(j * footSweepIncrements)][7])
            yLeg7.append(yVals[str(j * footSweepIncrements)][7])
            xLeg12.append(xVals[str(j * footSweepIncrements)][17])
            yLeg12.append(yVals[str(j * footSweepIncrements)][17])
